...
// Convert raw lux to ZCL MeasuredValue: MeasuredValue = 10000 * log10(lux) + 1
static uint16_t zb_lux_to_illuminance(float lux)
{
    if (lux <= 0.0001f) {
        return 0; // Unknown or extremely dark
    }
...
static float current_lux = 100.0f;
...
static void button_event_handler(switch_driver_handle_t handle)
{
        ...
        .cmd_ctrl =
            {
                ...
                .src_ep             = ESP_ZIGBEE_HA_LIGHT_SENSOR_EP_ID,
                .cluster_id         = EZB_ZCL_CLUSTER_ID_ILLUMINANCE_MEASUREMENT,
            },
        .payload =
            {
                .attr_id = EZB_ZCL_ATTR_ILLUMINANCE_MEASUREMENT_MEASURED_VALUE_ID,
            },
    };
    ...
    if (ret == EZB_ERR_NONE) {
        ESP_LOGI(TAG, "Sent ZCL Report Attribute request (illuminance: %.1f lux)", current_lux);
    } else {
        ESP_LOGE(TAG, "Failed to send ZCL Report Attribute request with error(0x%04x)", ret);
    }
}

static void simulated_light_sensor_task(void *pvParameters)
{
    bool increasing = true;
    for (;;) {
        // Simulate light level fluctuation (between 10 lux and 5000 lux)
        if (increasing) {
            current_lux += 150.0f;
            if (current_lux >= 5000.0f) {
                current_lux = 5000.0f;
                increasing = false;
            }
        } else {
            current_lux -= 150.0f;
            if (current_lux <= 10.0f) {
                current_lux = 10.0f;
                increasing = true;
            }
        }

        uint16_t measured_value = zb_lux_to_illuminance(current_lux);
        ESP_LOGI(TAG, "Simulated Light Sensor: %.1f lux -> ZCL value %u", current_lux, measured_value);

        /* Update light sensor measured value */
        esp_zigbee_lock_acquire(portMAX_DELAY);
        ezb_zcl_set_attr_value(ESP_ZIGBEE_HA_LIGHT_SENSOR_EP_ID, EZB_ZCL_CLUSTER_ID_ILLUMINANCE_MEASUREMENT,
                               EZB_ZCL_CLUSTER_SERVER, EZB_ZCL_ATTR_ILLUMINANCE_MEASUREMENT_MEASURED_VALUE_ID,
...
static esp_err_t deferred_driver_init(void)
{
    ...
    if (xTaskCreate(simulated_light_sensor_task, "sim_light_sensor", 3072, NULL, 5, NULL) != pdTRUE) {
        ESP_LOGE(TAG, "Failed to create simulated light sensor task");
        return ESP_FAIL;
    }
...
esp_err_t esp_zigbee_create_zha_light_sensor_device(void)
{
    ezb_af_device_desc_t          dev_desc         = ezb_af_create_device_desc();
    ezb_zha_light_sensor_config_t light_sensor_cfg = EZB_ZHA_LIGHT_SENSOR_CONFIG();
    light_sensor_cfg.illuminance_cfg.min_measured_value = 1;
    light_sensor_cfg.illuminance_cfg.max_measured_value = 65534;

    ezb_af_ep_desc_t ep_desc =
        ezb_zha_create_light_sensor(ESP_ZIGBEE_HA_LIGHT_SENSOR_EP_ID, &light_sensor_cfg);
...
static void esp_zigbee_stack_main_task(void *pvParameters)
{
    ...
    ESP_ERROR_CHECK(esp_zigbee_create_zha_light_sensor_device());
